...
#2017-2019 Happiness Country	
cur.execute("CREATE TABLE Happiness7(Country VARCHAR, \"Happiness.Rank\" SERIAL PRIMARY KEY, \"Happiness.Score\" float, \"Whisker.high\"	VARCHAR,\"Whisker.low\" VARCHAR, \"Economy..GDP.per.Capita.\" VARCHAR,\"Family\" VARCHAR, \"Health..Life.Expectancy.\" VARCHAR, \"Freedom\" VARCHAR, \"Generosity\" VARCHAR, \"Trust..Government.Corruption.\" VARCHAR, \"Dystopia.Residual\" VARCHAR);")
cur.execute("COPY Happiness7 FROM '/home/arman/D/happiness report (2015-2020)/2017.csv' WITH (format csv, header);")
...
cur.execute("CREATE TABLE Happiness9(\"Overall Rank\" SERIAL PRIMARY KEY, \"Country or Region\" TEXT, \"Score\" float, \"GDP per Capita\" float, \"Social Support\" float, \"Healthy Life Expectancy\" float, \"Freedom to make Life Choices\" float, \"Generosity\" float, \"Perceptions of Corruption\" VARCHAR);")
cur.execute("COPY Happiness9 FROM '/home/arman/D/happiness report (2015-2020)/2019.csv' WITH (format csv, header);")
...
cur.execute("CREATE TABLE Land AS SELECT DISTINCT \"Country Name\" AS Name, \"Country Code\" AS Id,P.\"Happiness.Rank\" AS \"Happiness Rank 17\", H.\"Overall Rank\" AS \"Happiness Rank 18\", \"Happiness.Score\" AS \"Happiness Score 17\",H.\"Score\" AS \"Happiness Score 18\", A.\"Overall Rank\" AS \"Happiness Rank 19\", A.\"Score\" AS \"Happiness Score 19\" FROM population_growth, (SELECT \"Overall Rank\",\"Score\",\"Country or Region\" FROM Happiness) H, (SELECT \"Overall Rank\",\"Score\",\"Country or Region\" FROM Happiness9) A,(SELECT \"Happiness.Rank\", \"Happiness.Score\",Country FROM Happiness7) P WHERE \"Country Name\" = H.\"Country or Region\" AND H.\"Country or Region\" = A.\"Country or Region\" AND A.\"Country or Region\"=P.Country;")

cur.execute("CREATE TABLE DatenOfLand AS SELECT P.\"Country Name\" AS Name, P.\"Country Code\" AS Id, P.Year AS Year, G.\"gdp\" , Count AS PopTotal, \"Annual CO₂ emissions (tonnes )\" AS \"Co2/Jahr\" FROM PopGrowth P, GdpNew G, co2_emission E, population_total T WHERE P.\"Country Name\" = E.Entity AND P.Year = E.Year AND E.Entity = T.\"Country Name\" AND E.Year = T.Year AND G.\"Country Name\" = T.\"Country Name\" AND G.Year = T.Year;")
...
cur.execute("SELECT L.Name, CASE WHEN L.Name = F.Name THEN \'Organizer\' WHEN L.Name =  First THEN \'Winner\' WHEN L.Name = Second THEN \'Second\' ELSE \'Third\' END AS COL, \"Happiness Rank 17\", \"Happiness Rank 18\", \"Happiness Rank 19\", \"Happiness Score 17\", \"Happiness Score 18\", \"Happiness Score 19\" FROM (SELECT Name, First, Second, Third FROM FifaWorldCup WHERE Year = \'2018\') F, LAND L WHERE L.Name = First OR L.Name = Second OR L.Name = Third OR L.Name = F.Name;")
table_2018 = cur.fetchall()
x1 = ["17 Rank",  "18 Rank", "19 Rank"] 
x2 = ["17 Score", "18 Score", "19 Score"]
fig, axs = plt.subplots(2, 4)
axs[0, 0].bar(x1, [161 - table_2018[0][i] for i in range(2,len(table_2018[0])-3)])
axs[0, 0].set_title(table_2018[0][0] +": "+table_2018[0][1]+" Happiness")
axs[0, 0].set_ylim([0,160])
axs[0, 1].bar(x2, [table_2018[0][i] for i in range(5,len(table_2018[0]))])
axs[0, 1].set_title(table_2018[0][0] +": "+table_2018[0][1]+" Happiness")
axs[0, 2].bar(x1, [161 - table_2018[1][i] for i in range(2,len(table_2018[1])-3)])
axs[0, 2].set_title(table_2018[1][0] +": "+table_2018[1][1]+" Happiness")
axs[0, 2].set_ylim([0,160])
axs[0, 3].bar(x2, [table_2018[1][i] for i in range(5,len(table_2018[1]))])
axs[0, 3].set_title(table_2018[1][0] +": "+table_2018[1][1]+" Happiness")
axs[1, 0].bar(x1, [161 - table_2018[2][i] for i in range(2,len(table_2018[2])-3)])
axs[1, 0].set_title(table_2018[2][0] +": "+table_2018[2][1]+" Happiness")
axs[1, 0].set_ylim([0,160])
axs[1, 1].bar(x2, [table_2018[2][i] for i in range(5,len(table_2018[2]))])
axs[1, 1].set_title(table_2018[2][0] +": "+table_2018[2][1]+" Happiness")
axs[1, 2].bar(x1, [161 - table_2018[3][i] for i in range(2,len(table_2018[3])-3)])
axs[1, 2].set_title(table_2018[3][0] +": "+table_2018[3][1]+" Happiness")
axs[1, 2].set_ylim([0,160])
axs[1, 3].bar(x2, [table_2018[3][i] for i in range(5,len(table_2018[3]))])
axs[1, 3].set_title(table_2018[3][0] +": "+table_2018[3][1]+" Happiness")

plt.show()
plt.cla()
plt.clf()

cur.execute("CREATE TABLE WC_Unpivot AS SELECT Year, unnest(array[\'Name\',\'First\',\'Second\',\'Third\']) AS Pos, unnest(array[Name,First,Second,Third]) AS Name FROM FifaWorldCup;")
cur.execute("SELECT Year, Pos ,Name, gdp/PopTotal FROM DatenOfLand NATURAL JOIN WC_Unpivot ORDER BY Year, Pos;")
newtab = cur.fetchall()
x_names = ["Winner", "Organizer", "Second", "Third"]

Liste = ['1930','1934','1938','1950','1954','1958','1962','1966','1970','1974','1978','1982','1986','1990','1994','1998','2002','2006','2010','2014','2018']
print("Bitte geben sie das Jahr einer Fussballweltmeisterschaft an")

user_input = input()
test = user_input.isnumeric()

print( type(user_input))

if test == True :
    print("Die Eingabe wurde akzepiert")
    exists = user_input in Liste
    print(exists)
    if exists == True:
        #print([newtab[i] for i in range(len(newtab)) if newtab[i][0] == int(user_input)])
        plt.title('GDP per capita of ' + str(user_input) + ' WC Podium')
        plt.ylabel('GDP per Capita of ' + str(user_input) + ' WC Podium')
        reszip = zip(x_names, [newtab[i][2] for i in range(len(newtab)) if newtab[i][0] == int(user_input)])
        result = ['' for x in range(len(x_names))]
        restemp = list(reszip)
        for r in range(len(restemp)):
            result[r] = restemp[r][0] + ': ' + restemp[r][1]
        plt.bar(result, [newtab[i][3] for i in range(len(newtab)) if newtab[i][0] == int(user_input)])
        plt.show()
    else:
        print("Bitte geben sie eines der Folgenden Jahre an:")
        print("1930,1934,1938,1950,1954,1958,1962,1966,1970,1974,1978,1982,1986,1990,1994,1998,2002,2006,2010,2014,2018")
